 ...
 * produce 'income' or 'expense'. Promote outflows in a subscription category so the
 * Subscriptions view does not have to understand two shapes. 'subscription' is
 * already in OUTFLOW_TYPES, so no total, quest, budget or chart figure changes.
 */
const resolveType = (baseType, category) =>
    baseType !== 'income' && SUBSCRIPTION_CATEGORIES.includes(category)
        ? 'subscription'
        : baseType;
