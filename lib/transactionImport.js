 ...
 * produce 'income' or 'expense'. Two refinements for outflows:
 *
 * - a subscription category promotes it to 'subscription', so the Subscriptions view
 *   does not have to understand two shapes. That type is already in OUTFLOW_TYPES,
 *   so no total, quest, budget or chart figure changes;
 * - 'Credit Card Payment' makes it a 'transfer'. Paying the card from checking moves
 *   money between the user's own accounts -- the spending is the card purchases, so
 *   counting the payment too would count every card dollar twice.
 */
const resolveType = (baseType, category) => {
    if (baseType === 'income') return baseType;
    if (category === 'Credit Card Payment') return 'transfer';
...
const CARD_COST_ROW = /\bINTEREST CHARGE\b|\bFEE\b/i;
...
const parseChaseCardStatementText = (text) => {
    ...
    const rows = blocks.map((block) => {
        ...
        // Counted on the printed line, not the cleaned name, so it is stable however
        // the name is later edited.
        const lineKey = `${formatDateForInput(date)}|${block.signedAmount}|${description.toLowerCase()}`;
