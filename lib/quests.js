...
// The Transaction type enum is ['income','expense','subscription','transfer'].
// Subscriptions are money leaving the account, so every aggregate here buckets them
// with expenses. Transfers (paying the card from checking) are in neither bucket --
// the spending is already counted as the card purchases themselves.
const { fromCents, isOverBy, roundMoney, toCents } = require('./money');
...
const summarize = (transactions) => {
    ...
    transactions.forEach((transaction) => {
        const cents = toCents(amountOf(transaction));
...
const spendByCategory = (transactions) => {
    ...
    transactions.filter(isOutflow).forEach((transaction) => {
        ...
        cents[key] = (cents[key] || 0) + toCents(amountOf(transaction));
    });
...
const QUEST_DEFINITIONS = [
    ...
    },
    {
        ...
        evaluate: ({ summary }) => {
            ...
            return {
                value: roundMoney(Math.max(0, summary.net)),
    ...
    },
    {
        ...
        evaluate: ({ transactions, budgets }) => {
            ...
            // Compared at cent precision: a cap met to the penny is not a breach.
            const within = caps.filter(([category, limit]) => !isOverBy(spend[category] || 0, limit)).length;
            return {
                value: within,
                target: caps.length,
